...
for i in range(int(RECORD_TIME*2*(RATE/CHUNK))): #do this for 10 seconds
    ...
    output = np.int16(audinterp(np.linspace(0,audio.shape[0]-1,int(audio.shape[0]*0.6))))
    output = np.append(output,output[::-1][:audio.shape[0]-output.shape[0]-1])
    for index in np.arange((int(audio.shape[0]*0.1))):
        wet = index/(audio.shape[0]*0.1)
